...
g.pt.plants.pglm.nobl = ggplot(valid.df, aes(x=pglm.coef, y=log(-log(pglm.pval)), label=label, color=p.cat)) + 
  geom_point() + geom_text_repel(max_overlaps=50, size=3, lineheight=0.75) + 
  theme_light() + labs(title="Plants PT gene count PGLM, no branch lengths", x ="PGLM coefficient", y = "log(-log(p))", color="p profile")
g.pt.plants.plm.nobl  = ggplot(valid.df, aes(x=plm.coef, y=log(-log(plm.pval)), label=label, color=p.cat)) + 
  geom_point() + geom_text_repel(max_overlaps=50, size=3, lineheight=0.75) + 
  theme_light() + labs(title="Plants PT gene count PLM, no branch lengths", x ="PLM coefficient", y = "log(-log(p))", color="p profile")

grid.arrange(g.pt.plants.pglm.nobl , g.pt.plants.plm.nobl )

sf = 2
png("pt-plants-nobl.png", width=600*sf, height=400*sf, res=72*sf)
grid.arrange(g.pt.plants.pglm.nobl , g.pt.plants.plm.nobl )
dev.off()
