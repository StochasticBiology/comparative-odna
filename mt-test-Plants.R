...
g.mt.plants.pglm = ggplot(valid.df, aes(x=pglm.coef, y=log(-log(pglm.pval)), label=label, color=p.cat)) + 
  geom_point() + geom_text_repel(max_overlaps=50, size=3, lineheight=0.75) + 
  theme_light() + labs(title="Plants MT gene count PGLM", x ="PGLM coefficient", y = "log(-log(p))", color="p profile")

g.mt.plants.plm = ggplot(valid.df, aes(x=plm.coef, y=log(-log(plm.pval)), label=label, color=p.cat)) + 
  geom_point() + geom_text_repel(max.overlaps=50, size=3, lineheight=0.75)+ 
  theme_light() + labs(title="Plants MT gene count PLM", x ="PLM coefficient", y = "log(-log(p))", color="p profile")

grid.arrange(g.mt.plants.pglm, g.mt.plants.plm)

sf = 2
png("mt-plants.png", width=600*sf, height=400*sf, res=72*sf)
grid.arrange(g.mt.plants.pglm, g.mt.plants.plm)
dev.off()
