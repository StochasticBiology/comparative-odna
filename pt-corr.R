  ...
  for(positive.label in label.set) {
    if(is.factor(label.set) & positive.label != "" & !is.na(positive.label)) {
      ...
      mydf2 = data.frame()
        # loop through leaves in taxonomy tree
          ...
          for(i in 1:length(thesenames)) {
            ...
            if(length(ref) > 0) {
              ...
              mydf2 = rbind(mydf2, data.frame(label=thesenames[i], x=as.factor(this.x), y=df$countsPT[ref]))
            }
          }
        ...
        write.csv(results.df, "mac-results-df-pt.csv", quote=F, row.names=FALSE)
