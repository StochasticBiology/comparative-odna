...
g.mt.orf.pglm = ggplot(valid.df, aes(x=pglm.coef, y=log(-log(pglm.pval)), label=label, color=p.cat)) + 
...
g.mt.orf.plm = ggplot(valid.df, aes(x=plm.coef, y=log(-log(plm.pval)), label=label, color=p.cat)) + 
  geom_point() + geom_text_repel(max.overlaps=50, size=3, lineheight=0.75)+
  theme_light() + labs(title="MT ORF count PLM", x ="PLM coefficient", y = "log(-log(p))", color="p profile")
grid.arrange(g.mt.orf.pglm, g.mt.orf.plm)

sf = 2
png("mt-test-ncbi.png", width=600*sf, height=400*sf, res=72*sf)
grid.arrange(g.mt.orf.pglm, g.mt.orf.plm)
dev.off()
...
png("mt-test-specifics-ncbi.png", width=600, height=400)
