...
for(clade in unique(df$clade)) {
  this.mean = mean(df$countsPT[df$clade==clade])
  df$shiftedPT[df$clade==clade] = df$countsPT[df$clade==clade]-this.mean
}
...
for(positive.column in column.set[-not.interesting]) {
  ...
  for(positive.label in label.set) {
    if(is.factor(label.set) & positive.label != "" & !is.na(positive.label)) {
      ...
      mydf2 = data.frame(label=df$Scientific.Name, x=ifelse(df[,positive.column]==positive.label, 1, 0), y=df$shiftedPT, clade=df$clade)
...
png("pt-test-clademean-specifics.png", width=600, height=400)
