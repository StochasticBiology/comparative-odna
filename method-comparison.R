      ...
      for(model.correlated in c(0,5,10,20)) {     # strength of predictor-response effect
        for(p01 in c(0)) {                        # false positive observation rate (0 for us)
          for(p10 in c(0,0.1,0.25,0.5)) {         # false negative observation rate
            cat(paste(c(reversible, tree.size, mean.events, model.correlated, p01, p10, "\n"), collapse=" "))
            for(this.expt in 1:20) {
              ...
              e.count = rbind(e.count, data.frame(reversible=reversible, tree.size=tree.size, mean.events=mean.events, model.correlated=model.correlated, p01=p01, p10=p10, expt=expt, e.01=e.01, e.10=e.10))
              ...
              basic.pval = pgls.pval = pglm.pval = plm.pval = wilcox.pval = boot.pval = NA
...
for(this.rev in c(0,1)) {
  ...
  g.2 = ggplot(pvals[pvals$reversible==this.rev & pvals$p01==0,], aes(x=factor(model.correlated), y=log10(-log10(basic.pval)), color=noise.label)) + 
    geom_boxplot() + geom_hline(yintercept=log10(-log10(0.05)), color="#888888")+ 
    geom_vline(xintercept=1.5, color="#888888")+
    geom_text(data=label.text,aes(x=x,y=y,label=label),color="#888888",size=3) +
    facet_grid(mean.events~tree.size) +
  ...
  g.3 = ggplot(pvals[pvals$reversible==this.rev & pvals$p01==0,], aes(x=factor(model.correlated), y=log10(-log10(wilcox.pval)), color=noise.label)) + 
    geom_boxplot() + geom_hline(yintercept=log10(-log10(0.05)), color="#888888")+ 
    geom_vline(xintercept=1.5, color="#888888")+
    geom_text(data=label.text,aes(x=x,y=y,label=label),color="#888888",size=3) +
    facet_grid(mean.events~tree.size) + 
  ...
  g.4 = ggplot(pvals[pvals$reversible==this.rev & pvals$p01==0,], aes(x=factor(model.correlated), y=log10(-log10(boot.pval)), color=noise.label)) + 
    geom_boxplot() + geom_hline(yintercept=log10(-log10(0.05)), color="#888888")+ 
    geom_vline(xintercept=1.5, color="#888888")+
    geom_text(data=label.text,aes(x=x,y=y,label=label),color="#888888",size=3) +
    facet_grid(mean.events~tree.size) + 
  ...
  # PGLM
  g.5 = ggplot(pvals[pvals$reversible==this.rev & pvals$p01==0,], aes(x=factor(model.correlated), y=log10(-log10(pglm.pval)), color=noise.label)) + 
    geom_boxplot() + geom_hline(yintercept=log10(-log10(0.05)), color="#888888")+ 
    geom_vline(xintercept=1.5, color="#888888")+
    geom_text(data=label.text,aes(x=x,y=y,label=label),color="#888888",size=3) +
    facet_grid(mean.events~tree.size) + 
  ...
  # PLM
  g.6 = ggplot(pvals[pvals$reversible==this.rev & pvals$p01==0,], aes(x=factor(model.correlated), y=log10(-log10(plm.pval)), color=noise.label)) + 
    geom_boxplot() + geom_hline(yintercept=log10(-log10(0.05)), color="#888888")+ 
    geom_vline(xintercept=1.5, color="#888888")+
    geom_text(data=label.text,aes(x=x,y=y,label=label),color="#888888",size=3) +
    facet_grid(mean.events~tree.size) + 
  ...
  sf = 2
  fname = paste(c("method-comparison-rev-", this.rev, ".png"), collapse="")
  png(fname, width=1000*sf, height=1000*sf, res=72*sf)
  print(grid.arrange(g.1, g.6, g.5, g.2, g.3, g.4, nrow=3))
  dev.off()
  ...
  fname = paste(c("method-comparison-obs-rev-", this.rev, ".png"), collapse="")
  png(fname, width=600, height=600)
  print(ggplot(obs.stats[pvals$reversible==this.rev,], aes(x = mean.events, y=obs1/(obs0+obs1))) + geom_violin() + facet_grid(mean.events ~ tree.size))
  dev.off()
}
