...
g.pt.orf.pglm = ggplot(valid.df, aes(x=pglm.coef, y=log(-log(pglm.pval)), label=label, color=p.cat)) + 
...
g.pt.orf.plm = ggplot(valid.df, aes(x=plm.coef, y=log(-log(plm.pval)), label=label, color=p.cat)) + 
  geom_point() + geom_text_repel(max.overlaps=50, size=3, lineheight=0.75) + 
  theme_light() + labs(title="PT ORF count PLM", x ="PLM coefficient", y = "log(-log(p))", color="p profile")
grid.arrange(g.pt.orf.pglm, g.pt.orf.plm)
