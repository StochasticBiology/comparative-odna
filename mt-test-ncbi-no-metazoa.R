...
for(positive.column in column.set[-not.interesting]) {
  ...
  for(positive.label in label.set) {
    if(is.factor(label.set) & positive.label != "" & !is.na(positive.label)) {
      ...
      expt.label = paste(c(colnames(df)[positive.column], ": ", positive.label, " in ", tree.labels[MRCA] ), collapse="")
      cor.plot[[length(cor.plot)+1]] = ggplot(mydf2, aes(x=x,y=y)) + 
        geom_jitter(width=0.2, alpha=0.2) + geom_smooth(method="lm") + 
        geom_boxplot(aes(x=ifelse(x==0,-0.3,1.3), group=factor(x)), width=0.1) + ggtitle(expt.label) + theme(plot.title = element_text(size = 12))
       # cor.plot[[length(cor.plot)+1]] = ggplot(mydf2, aes(x=x,y=y)) + 
...
g.mt.orf.nomet.pglm = ggplot(valid.df, aes(x=pglm.coef, y=log(-log(pglm.pval)), label=label, color=p.cat)) + 
  geom_point() + geom_text_repel(max.overlaps=50, size=3, lineheight=0.75) +
  theme_light() + labs(title="MT ORF count PGLM", x ="PGLM coefficient", y = "log(-log(p))", color="p profile")
g.mt.orf.nomet.plm = ggplot(valid.df, aes(x=plm.coef, y=log(-log(plm.pval)), label=label, color=p.cat)) + 
  geom_point() + geom_text_repel(max.overlaps=50, size=3, lineheight=0.75) +
  theme_light() + labs(title="MT ORF count PLM", x ="PLM coefficient", y = "log(-log(p))", color="p profile")
grid.arrange(g.mt.orf.nomet.pglm, g.mt.orf.nomet.plm)

sf = 2
png("mt-test-ncbi-no-metazoa.png", width=600*sf, height=400*sf, res=72*sf)
grid.arrange(g.mt.orf.nomet.pglm, g.mt.orf.nomet.plm)
dev.off()
...
png("mt-test-specifics-ncbi-no-metazoa.png", width=600, height=400)
